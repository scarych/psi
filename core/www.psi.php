...
class PSI_Www extends PSI_Core {
    ...
    //-- установка или возврат первичной ссылки
    ...
    //-- следующее значение по ссылке
    public function next($default = null) {
        return ($return = array_shift($this->_url)) ? $return : $default;
    }
    //-- последнее значение по ссылке
    public function last($default = null) {
        return ($return = array_pop($this->_url)) ? $return : $default;
    }
    ...
    //-- сборка текущего местположения на основе веток атомов
    public function __toString() {
        $this->__toString = true;
        $process = $this->_process;
        $return = array();
        while ($this->_process) { //-- сборка состояний
            $return[] = call_user_func($this->_process);
        }
        //-- возврат точки сборки на место
    ...
    public function __invoke() {
        list($class, $args) = sgra(__CLASS__);
...
//    //-- IN PROCESS
//    public function request($addr = '/', $full = false) {
//        return $this;
//    }
//
...
class PSI_Atom extends PSI {
    ...
    public function __call($function, $arguments = array()) {
        if (isset($this->_quants[$function])) {
            ...
            $this->_quant($function, function () use ($arguments, $function) {
                 ...
                 *
                 */
                list($psi) = tail($args = func_get_args(), null); //-- todo
                $atom = $psi();
    ...
    */
    ...
    public function __invoke() { //-- аргумент тут всегда или один, или ни одного
        if (!is_null($argument = array_pop(func_get_args()))) { //-- если есть входящие аргументы, то считаем, что вызывается функция на обработку погружения, и рассматриваем ее
            if ($argument===$this) {
